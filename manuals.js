/* Field CRM - offline engineering manual library.
   Ported unchanged from Belt Call Log v13 except for the database name.
   ...
   The database is 'fieldcrmmanuals', NOT the Belt Call Log's 'beltmanuals'.
   GitHub Pages puts every repo on one origin, so the two apps share one. Reading
   the old app's database would mean the Belt Call Log could clear pages this app
   is relying on, and an uninstall of either would take both. The cost is loading
   the manuals once more here; the manuals themselves are unchanged.

   Storage is a SEPARATE database from the calls on purpose:
   ...
   - it needs no coordination with app.js's DB_VER, so adding this cannot
     trigger an upgrade of the fieldcrm database and cannot lose calls
...
(function () {
  ...
  var MDB = 'fieldcrmmanuals', MDB_VER = 1;
